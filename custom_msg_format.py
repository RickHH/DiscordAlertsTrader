...
def msg_custom_formated(message, bksession=None):
    "Custom message format for specific channels"
    time_strf = "%Y-%m-%d %H:%M:%S.%f"

    # --- AbTrades handler ---
    if message.channel.id == 986816019295252500:
        ...
        if not content.startswith("$"):
            return None
        # skip recaps: multi-line summaries with Unrealized/Realized headers
        if any(kw in content for kw in ["Unrealized", "Realized", "Position Updates", "Swings:"]):
            return None
        # skip multi-line recaps (more than one $ trade line)
        dollar_lines = [l for l in content.split("\n") if l.strip().startswith("$")]
        if len(dollar_lines) > 1:
            return None
        content = content[1:].strip()
        main_part = re.split(r"\s*[\(\[]", content)[0].strip()
