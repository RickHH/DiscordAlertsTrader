...
def abi_formatting(message_):
    ...
    if match:
        ...
        formatted_alert = f"BTO {ticker.upper()} {strike}{otype.upper()} {exp_date} @{str(round(float(price)*1,2))}"
        message.content = formatted_alert
...
def gandalf_formatting(message_):
    ...
    message = MessageCopy(message_)
    alert = message.content
    pattern = r'BTO\s+([A-Z]+)\s+\$(\d+)([cCpP])\s+(\d{1,2}/\d{1,2})\s+@\s+\$\s+([\d.]+)'
    match = re.search(pattern, alert, re.IGNORECASE)
    if match:
...
#     Reformat Discord message from Adex_swing
#     """
#     alert = 'Entered: $ENPH 60P 3/14 @2.40'
#     pattern = r'\$([A-Z]+)\s+(\d+)([cCpP])\s+(\d{1,2}/\d{1,2})\s+@([\d.]+)'
#     match = re.search(pattern, alert, re.IGNORECASE)
